...
export default function GitBackupPage() {
  ...
  async function handleCommit() {
    ...
    try {
      ...
      notify.success(
        t("ui.git.commit_ok", "Commit erstellt") + `: ${entry.short_hash}`,
      );
      setMessage("");
      await refresh();
    } catch (err) {
      if (
        ...
        err.detailBody?.code === "nothing_to_commit"
      ) {
        notify.warning(
          t("ui.git.nothing_to_commit", "Keine Änderungen zu committen"),
        );
        return;
      }
      notify.error(describeError(err), err);
  ...
  async function handleSaveRemote() {
    if (!remoteUrlDraft.trim()) {
      notify.warning(
        t("ui.git.remote_url_required", "Remote-URL erforderlich"),
      );
      return;
    }
    setBusy(true);
    try {
      await api.git.setRemote(
        bookId,
        remoteUrlDraft.trim(),
        remotePatDraft || null,
      );
  ...
  async function handlePush(force: boolean = false) {
    ...
    try {
      ...
      if (
        err instanceof ApiError &&
        err.detailBody?.code === "remote_rejected"
      ) {
  ...
  async function handleMergeRemote() {
    ...
    try {
      ...
      if (res.status === "conflicts") {
        ...
        notify.warning(
          t("ui.git.conflicts_found", "Konflikte gefunden — wähle pro Datei"),
        );
      }
    } catch (err) {
      notify.error(describeError(err), err);
                  ...
                  >
                    <div className="field">
                      <label className="label">
                        {t("ui.git.remote_url", "Remote-URL")}
                      </label>
                      <input
                        className="input"
      ...
      {inResolution ? (
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 8, flexWrap: "wrap" }}>
            ...
            >
              <Check size={14} />{" "}
              {t("ui.git.apply_resolution", "Auflösung anwenden")}
            </button>
            <button
              className="btn btn-ghost btn-sm"
              onClick={onAbort}
