...
vi.mock("../api/client", () => ({
  api: {
    ...
    books: {
      ...
      createFromTemplate: (...args: unknown[]) =>
        mockBooksCreateFromTemplate(...args),
    },
  },
...
describe("CreateBookPage", () => {
  ...
  it("renders the generic title for prose with its testid", async () => {
    ...
    expect(screen.getByTestId("create-book-title-prose").textContent).toBe(
      "Neues Buch",
    )
  })
  ...
  it("renders the picture-book title from ?type=picture_book", async () => {
    ...
    expect(
      screen.getByTestId("create-book-title-picture_book").textContent,
    ).toBe("Neues Bilderbuch")
  })
