/**
 * Per-record write serialization for DexieStorage (PR #126).
 *
 * Every `update*` method reads a row, shallow-merges the patch, and writes
 * the whole row back. Without serialization, two concurrent calls on the
 * same record each read the pre-other-write state, and the later `put()`
 * (built from a stale read) drops the other call's field changes.
 */
