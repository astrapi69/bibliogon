/**
 * Typed DTOs for the Bibliogon API client.
 *
 * Extracted from the former 5212-line client.ts (Batch 2 god-file split).
 * Pure type declarations (plus the small `formatVoiceLabel` helper) with no
 * runtime/transport dependencies. The api/client.ts barrel re-exports all of
 * these, and the entity API modules import their DTOs from "./client" (which
 * resolves here), so every existing call site stays unchanged.
 */
...
export type StoryBeat = "setup" | "inciting" | "rising" | "climax" | "falling" | "resolution";
...
export interface Article {
    ...
    /** Dexie-only (#157): id of the article's featured image stored as
     *  bytes in the offline `articleAssets` table. Set by the offline
     *  upload path and the Medium-import CDN cache so the dashboard
     *  thumbnail survives offline; resolved by `useArticleImageUrl`.
     *  Absent / null in api mode (the server serves the file directly). */
...
export type PublicationStatus = "planned" | "scheduled" | "published" | "out_of_sync" | "archived";
...
export type ApplySkipReason = "value-is-empty" | "field-already-populated" | "all-entries-dropped";

export type ApplySkipReasons = Record<string, ApplySkipReason>;

export type ArticleFieldClass = "seo" | "tags" | "topic" | "excerpt" | "image_prompts";
...
export interface DroppedChapterSummary {
    reason: "no-matching-chapter" | "summary-empty" | "not-a-mapping" | string;
    chapter_id?: string | null;
    title?: string | null;
    entry?: unknown;
}
...
export type BulkAiFillJobStatusName = "pending" | "running" | "completed" | "failed" | "cancelled";
...
export type ReviewStatus = "invited" | "sent" | "received" | "reviewed" | "declined";
...
export interface BookPublishingStateApi {
    ...
    prices: Record<string, { currency: string; list_price: number; page_count?: number }>;
...
export type RelationshipType = "ally" | "rival" | "family" | "mentor" | "romantic" | "neutral";
...
export interface GitSyncResolveResult {
    counts: Record<"updated" | "created" | "deleted" | "marked" | "renamed" | "skipped", number>;
}
...
export type GitSyncSubsystemStatus = "ok" | "skipped" | "nothing_to_commit" | "failed";
