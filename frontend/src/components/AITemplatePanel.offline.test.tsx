...
// (ai-template-file-io) follow the same key-dependent gating per the
...
describe("AITemplatePanel offline (article)", () => {
  ...
  it("disables Export and Import offline without a key", () => {
    renderPanel("article", "a1", false);
    expect(
  ...
  it("enables Export and Import offline with a key (active per contract)", () => {
    renderPanel("article", "a1", true);
    expect(
      (screen.getByTestId("ai-template-export") as HTMLButtonElement).disabled,
    ).toBe(false);
