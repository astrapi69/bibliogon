...
export default function AudioExportProgress() {
  ...
  const handleCancel = async () => {
    ...
    try {
      ...
      notify.error(
        t("ui.audio_progress.cancel_failed", "Abbruch fehlgeschlagen") +
          ": " +
          detail,
        err,
      );
        ...
        >
          ...
          >
            ...
            <Dialog.Title className="dialog-title">
              {t("ui.audio_progress.title", "Audiobook wird generiert")}:{" "}
              {bookTitle}
            </Dialog.Title>
          </div>
          ...
          >
            {phase === "connecting" &&
              t("ui.audio_progress.connecting", "Verbinde...")}
            {phase === "running" &&
              (total > 0
                ? t(
                    "ui.audio_progress.chapter_x_of_y",
                    "Kapitel {current} von {total}",
                  )
                    .replace("{current}", String(current))
                    .replace("{total}", String(total))
                : t("ui.audio_progress.starting", "Starte Generierung..."))}
            {phase === "completed" &&
              t("ui.audio_progress.completed", "Audiobook fertig")}
            {phase === "cancelled" &&
              t("ui.audio_progress.cancelled", "Export abgebrochen")}
            {phase === "failed" &&
              (errorMessage ||
                t("ui.audio_progress.failed", "Generierung fehlgeschlagen"))}
          </div>
          ...
          >
            {downloadUrl && phase === "completed" && (
              ...
              >
                <Download size={16} />{" "}
                {t("ui.audio_progress.download_zip", "ZIP herunterladen")}
              </a>
            )}
            {cancellable && (
              <button
                ...
                onClick={minimize}
                title={t(
                  "ui.audio_progress.minimize_hint",
                  "Im Hintergrund weiterlaufen lassen",
                )}
              >
                <Minimize2 size={14} />{" "}
                {t("ui.audio_progress.minimize", "Im Hintergrund fortsetzen")}
              </button>
            )}
            {cancellable && (
              <button
...
function ChapterFileList({
  ...
  return (
    <details open style={{ marginTop: 16 }}>
      ...
      >
        {t("ui.audio_progress.individual_files", "Einzelne Kapitel")} (
        {files.length})
      </summary>
      <ul
        style={{
      ...
      >
        {files.map((f) => (
          <li
            key={f.filename}
            style={{ padding: "3px 0", fontSize: "0.8125rem" }}
          >
            <a
              href={f.url}
              download={f.filename}
...
function eventColor(type: AudiobookEventType): string {
  if (type === "chapter_done" || type === "merge_done" || type === "done")
    return "var(--text-secondary)";
  if (type === "chapter_reused") return "var(--success, #16a34a)";
  if (type === "chapter_error" || type === "merge_error")
...
export function eventLabel(
  ...
  const prefix =
    idx > 0 ? `${formatChapterPrefix(idx, total)} | ${title}` : title;
  switch (ev.type) {
    case "start":
    ...
    case "merge_start":
      return t(
        "ui.audio_progress.event_merge_start",
        "Kapitel werden zusammengefügt...",
      );
    case "merge_done":
    ...
    case "done": {
      const parts = [
        t("ui.audio_progress.event_finished", "Generierung abgeschlossen"),
      ];
      if (typeof d.reused === "number" && d.reused > 0) {
        parts.push(
          `${t("ui.audio_progress.event_reused_count", "Wiederverwendet")}: ${d.reused}`,
        );
      }
      if (typeof d.cost_usd === "number" && d.cost_usd > 0) {
        parts.push(
          `${t("ui.audio_progress.event_cost", "Kosten")}: ~$${d.cost_usd.toFixed(2)}`,
        );
      }
      if (typeof d.saved_usd === "number" && d.saved_usd > 0) {
        parts.push(
          `${t("ui.audio_progress.event_saved", "Gespart")}: ~$${d.saved_usd.toFixed(2)}`,
        );
      }
      return parts.join(" | ");
    }
