...
export default function DonationOnboardingDialog({
  open,
  onClose,
  donations,
}: Props) {
    ...
    >
      <Dialog.Portal>
        ...
        <Dialog.Content
          ...
          data-testid="donation-onboarding"
          aria-describedby={undefined}
        >
          <div className={styles.header}>
            <Dialog.Title className={styles.title}>
              <Heart size={18} aria-hidden />{" "}
              {t("ui.donations.onboarding_title", "Bibliogon unterstützen")}
            </Dialog.Title>
            <Dialog.Close asChild>
              <button
                className="btn-icon"
                aria-label={t("ui.common.close", "Schließen")}
                data-testid="donation-onboarding-close"
              >
                <X size={16} />
              </button>
            </Dialog.Close>
          ...
          {!showChannels ? (
            <>
              <p className={styles.body}>
                {t(
                  "ui.donations.onboarding_body",
                  "Bibliogon entsteht als Open-Source-Projekt ohne Tracking, ohne Cloud-Backend, ohne Werbung.",
                )}
              </p>
              <p className={styles.hint}>
                {t(
                  "ui.donations.onboarding_hint",
                  "Diesen Hinweis findest du jederzeit in den Einstellungen.",
                )}
              </p>
              <div className={styles.actions}>
                <button
                  ...
                  data-testid="donation-onboarding-support"
                >
                ...
                <button
                  ...
                  onClick={handleClose}
                  data-testid="donation-onboarding-understood"
                >
            ...
            <>
              <p className={styles.body}>
                {t(
                  "ui.donations.intro",
                  "Bibliogon entsteht als Open-Source-Projekt...",
                )}
              </p>
                  ...
                  >
                    ...
                    {channel.recommended ? (
                      <span className={styles.badge}>
                        ...
                        {t("ui.donations.recommended_badge", "Empfohlen")}
                      </span>
                    ) : null}
                  </a>
                ))}
