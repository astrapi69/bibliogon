...
function render(ui: React.ReactElement, options?: RenderOptions) {
    return rtlRender(ui, {
        wrapper: ({ children }) => (
            <BookTypesProvider initialTypes={TEST_BOOK_TYPES}>{children}</BookTypesProvider>
        ),
        ...options,
    });
}
...
describe("sanitizeAmazonHtml", () => {
    it("preserves allowed Amazon tags", () => {
        const html =
            "<b>bold</b> <i>italic</i> <em>em</em> <strong>strong</strong> <u>underline</u>";
...
describe("HtmlFieldWithPreview", () => {
    ...
    it("calls onChange when typing in textarea", () => {
        ...
        render(<HtmlFieldWithPreview label="Description" value="" onChange={handleChange} />);
    ...
    it("sanitizes dangerous HTML in preview", () => {
        render(
            <HtmlFieldWithPreview
                label="Test"
                value='<b>safe</b><script>alert("xss")</script>'
                onChange={() => {}}
            />,
        );
        fireEvent.click(screen.getByTestId("html-preview-toggle"));
...
describe("AuthorAssetsPanel", () => {
        ...
        assetsListMock.mockResolvedValue([
            {
                ...
                filename: "figure.png",
                asset_type: "figure",
                path: "uploads/book-y/figure/figure.png",
            },
            {
                id: "a2",
                filename: "portrait.png",
                asset_type: "author-asset",
                path: "uploads/book-y/author-asset/portrait.png",
            },
            {
                id: "a3",
                filename: "signature.png",
                asset_type: "author-asset",
                path: "uploads/book-y/author-asset/signature.png",
            },
        ]);
        render(<AuthorAssetsPanel bookId="book-y" />);
        await waitFor(() => expect(screen.getByTestId("author-assets-panel")).toBeInTheDocument());
    ...
    it("delete button removes asset from grid and calls api", async () => {
        assetsListMock.mockResolvedValue([
            {
                ...
                filename: "portrait.png",
                asset_type: "author-asset",
                path: "uploads/book-z/author-asset/portrait.png",
            },
        ]);
        ...
        await waitFor(() => expect(assetsDeleteMock).toHaveBeenCalledWith("book-z", "a1"));
        await waitFor(() =>
...
describe("BookMetadataEditor — Bug 9 Categories + BISAC", () => {
    ...
    it("does NOT render Categories + BISAC on initial mount (default 'general' tab)", () => {
        ...
        render(<BookMetadataEditor book={makeBook()} onSave={localOnSave} onBack={localOnBack} />);
    ...
    it("seeds CategoryInput + BisacCodeInput from book.categories + book.bisac_codes", () => {
        ...
        expect(screen.getByTestId("category-chip-0").textContent).toContain("Fiction");
        expect(screen.getByTestId("category-chip-1").textContent).toContain("Fantasy");
        expect(screen.getByTestId("bisac-chip-0").textContent).toContain("FIC022020");
        expect(screen.getByTestId("bisac-chip-1").textContent).toContain("BIO000000");
    });
...
describe("BookMetadataEditor — book_type tab filtering (Session 5 Commit 1)", () => {
    ...
    function renderEditor(bookOverrides: Partial<BookDetail> = {}) {
        return render(
            <BookMetadataEditor book={makeBook(bookOverrides)} onSave={onSave} onBack={onBack} />,
        );
    }
...
describe("BookMetadataEditor Design-tab Export-PDF button (picture_book only)", () => {
    ...
    it("picture_book: Export-PDF button is rendered in the Design tab", async () => {
        renderEditor({ book_type: "picture_book", chapters: [] });
        activateDesignTab();
        await waitFor(() => expect(screen.getByTestId("metadata-export-pdf")).toBeTruthy());
    });
    ...
    it("prose: Export-PDF button is NOT rendered in the Design tab (picture-book-only feature)", async () => {
        ...
        expect(screen.queryByTestId("metadata-export-pdf")).toBeNull();
    });

    it("clicking Export-PDF calls api.documentExport.download with bookId + 'pdf' + empty URLSearchParams", async () => {
        renderEditor({ book_type: "picture_book", chapters: [] });
        activateDesignTab();
        const btn = await screen.findByTestId("metadata-export-pdf");
        fireEvent.click(btn);
        await waitFor(() => expect(documentExportDownloadMock).toHaveBeenCalledTimes(1));
    ...
    it("ApiError surfaces as notify.error with the server's detail", async () => {
        ...
        const firstArg = (notify.error as ReturnType<typeof vi.fn>).mock.calls[0][0];
...
describe("BookMetadataEditor — Repository-URL field (BOOK-REPOSITORY-URL-FIELD-01)", () => {
    ...
    it("renders the read-only branch when git-sync mapping exists", async () => {
        ...
        expect(within(wrapper).getByTestId("metadata-repository-url-managed-hint")).toBeTruthy();
    });
