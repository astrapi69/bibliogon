...
export default function ChapterTemplatePickerModal({
  open,
  onClose,
  onInsert,
}: Props) {
  ...
  const [editingTemplate, setEditingTemplate] =
    useState<ChapterTemplate | null>(null);
    ...
    try {
      ...
      notify.error(
        err instanceof ApiError
          ? err.detail
          : t(
              "ui.chapter_template_picker.export_failed",
      ...
      try {
        ...
        notify.error(
          err instanceof ApiError
            ? err.detail
            : t(
                "ui.chapter_template_picker.import_failed",
  ...
  const handleDelete = async (tpl: ChapterTemplate) => {
    ...
    const ok = await dialog.confirm(
      ...
      t(
        "ui.chapter_template_picker.delete_confirm",
    ...
    try {
      ...
      notify.success(
        t("ui.chapter_template_picker.deleted", "Vorlage gelöscht"),
      );
    } catch (err) {
      notify.error(
        err instanceof ApiError
          ? err.detail
          : t(
              "ui.chapter_template_picker.delete_failed",
      ...
      >
          <DialogHeader>
            ...
            <button
              ...
              aria-label={t(
                "ui.chapter_template_picker.import",
                "JSON importieren",
              )}
          ...
          <div className={styles.body}>
            ...
            {templates !== null && templates.length === 0 && (
              <div className={styles.emptyState}>
                {templatesError
                  ? t(
                      "ui.chapter_template_picker.load_error",
                      "Vorlagen konnten nicht geladen werden",
                    )
                  : t(
                      "ui.chapter_template_picker.empty",
                      "Keine Kapitelvorlagen verfügbar",
                    )}
              </div>
            )}
            {templates !== null && templates.length > 0 && (
                      ...
                      <div className={styles.cardHeader}>
                        ...
                        <div className={styles.badges}>
                          {(tpl.child_template_ids?.length ?? 0) > 0 ? (
                            ...
                            >
                              {t(
                                "ui.chapter_template_picker.group_count",
                                "{count} Kapitel",
                              ).replace(
                                "{count}",
                                String(tpl.child_template_ids?.length ?? 0),
                              )}
                            </span>
                          ) : (
                          ...
                          <button
                            ...
                            aria-label={t(
                              "ui.chapter_template_picker.export",
                          ...
                          {tpl.is_builtin ? (
                            <span
                              ...
                              title={t(
                                "ui.chapter_template_picker.builtin_hint",
                            ...
                            >
                              <Lock size={10} />
                              {t(
                                "ui.chapter_template_picker.builtin",
                                "Mitgeliefert",
                              )}
                            </span>
                          ) : (
                            <>
                              <button
                                ...
                                className={`btn-icon ${styles.deleteBtn}`}
                                aria-label={t(
                                  "ui.chapter_template_picker.edit",
                                  "Bearbeiten",
                                )}
                              ...
                              <button
                                ...
                                className={`btn-icon ${styles.deleteBtn}`}
                                aria-label={t(
                                  "ui.chapter_template_picker.delete",
                                  "Löschen",
                                )}
