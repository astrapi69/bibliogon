...
export default function AiSetupWizard({
  ...
  const [baseUrl, setBaseUrl] = useState(
    AI_PROVIDER_PRESETS.anthropic.base_url,
  );
  const [model, setModel] = useState(
    AI_PROVIDER_PRESETS.anthropic.default_model,
  );
  ...
  const needsKey =
    preset?.requires_api_key !== false && !secretsManagedExternally;
          ...
          {step === 1 && (
            <div style={styles.stepContent}>
              {secretsManagedExternally ? (
                <div
                  className="field"
                  data-testid="wizard-api-key-external-note"
                >
...
export function shouldShowAiWizard(
