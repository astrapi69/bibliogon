...
interface Props {
    ...
    /** Called when the user cancels or an export hands off / completes.
     *  The page wires this to back-navigation (useGoBack). */
    onDone: () => void;
}
...
/**
 * Export configuration form (extracted from the former ExportDialog as
 * part of the Dialog->Pages migration C3). Chrome-free + self-contained:
 * the page (ExportPage) supplies the PageLayout shell + "Export: <title>"
 * heading and fetches the book; this form owns the format/options state
 * and performs the document download / audiobook hand-off directly.
 *
 * The nested regeneration-mode picker stays a small confirmation Dialog
 * (per the migration's "confirmation dialogs stay" rule).
 */
export default function ExportForm({bookId, bookTitle, hasManualToc, onDone}: Props) {
    const {t} = useI18n();
    ...
    // Track the live blob URL so it can be revoked on unmount (the page
    // unmounts on back-navigation, replacing the dialog's close cleanup).
    const dryRunUrlRef = useRef<string | null>(null);
    ...
    const handleDryRun = async () => {
        ...
        try {
            ...
            dryRunUrlRef.current = result.audioUrl;
    ...
    // Revoke the dry-run blob URL when the form unmounts (back-nav).
    useEffect(() => {
        return () => {
            if (dryRunUrlRef.current) URL.revokeObjectURL(dryRunUrlRef.current);
        };
    }, []);
    ...
    return (
        <>
            ...
            <div className="mt-6 flex flex-wrap justify-end gap-2 border-t border-border pt-4">
                <button className="btn btn-ghost" onClick={onDone}>{t("ui.common.cancel", "Abbrechen")}</button>
                <button
            ...
            {/* --- Regeneration mode dialog (confirmation; stays a Dialog) --- */}
