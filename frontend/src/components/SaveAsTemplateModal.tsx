...
export default function SaveAsTemplateModal({
  ...
  const defaultGenre = useMemo(
    () =>
      book.genre && TEMPLATE_GENRES.includes(book.genre)
        ? book.genre
        : "nonfiction",
    [book.genre],
  );
  ...
  const handleSubmit = async () => {
    ...
    if (!trimmedName) {
      setNameError(
        t("ui.save_template.name_required", "Name ist erforderlich"),
      );
      return;
    }
    if (!trimmedDescription) {
      notify.error(
        t(
          "ui.save_template.description_required",
          ...
          <div className={styles.body}>
            <div className="field">
              ...
              <input
                ...
                placeholder={t(
                  "ui.save_template.name_placeholder",
                  "z.B. Mein Memoir-Muster",
                )}
              ...
              {nameError && (
                <div
                  ...
                  data-testid="save-template-name-error"
                >
                  {nameError}
                </div>
              )}
            ...
            <div className="field">
              <label className="label">
                {t("ui.save_template.description", "Beschreibung")} *
              </label>
              <EnhancedTextarea
                value={description}
                onChange={setDescription}
                placeholder={t(
                  "ui.save_template.description_placeholder",
            ...
            <div className={styles.row}>
              ...
              <div className="field" style={{ flex: 1 }}>
                <label className="label">
                  {t("ui.save_template.language", "Sprache")}
                </label>
                <Select.Root value={language} onValueChange={setLanguage}>
                  <Select.Trigger
                    className="radix-select-trigger"
                    data-testid="save-template-language"
                  >
                    <Select.Value />
                    <Select.Icon>
                      <ChevronDown size={14} />
            ...
            <div className="field">
              <label className="label">
                {t("ui.save_template.content_mode", "Kapitelinhalt")}
              </label>
              <label className={styles.radioRow}>
                ...
                <div>
                  <div>
                    {t("ui.save_template.content_empty", "Leere Platzhalter")}
                  </div>
                  <div className={styles.hint}>
                    {t(
                      "ui.save_template.content_empty_hint",
              ...
              <label className={styles.radioRow}>
                ...
                <div>
                  <div>
                    {t(
                      "ui.save_template.content_preserve",
                      "Inhalt übernehmen",
                    )}
                  </div>
                  <div className={styles.hint}>
                    {t(
                      "ui.save_template.content_preserve_hint",
                      "Kopiert den Kapiteltext in die Vorlage",
                    )}
                  </div>
                </div>
              </label>
            ...
            <Collapsible.Root open={previewOpen} onOpenChange={setPreviewOpen}>
              <Collapsible.Trigger asChild>
                <button
                  ...
                  data-testid="save-template-preview-toggle"
                >
                  {previewOpen ? (
                    <ChevronDown size={14} />
                  ) : (
                  ...
                  {t("ui.save_template.preview", "Kapitelvorschau")} (
                  {chapterCount})
                </button>
              </Collapsible.Trigger>
              <Collapsible.Content>
                <div
                  ...
                  data-testid="save-template-preview-list"
                >
                  {book.chapters
                    .slice()
                    ...
                    .map((c) => (
                      <div key={c.id} className={styles.previewRow}>
                        <span className={styles.previewPos}>
                          {c.position + 1}.
                        </span>
