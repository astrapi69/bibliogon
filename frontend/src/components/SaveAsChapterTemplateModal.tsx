...
export default function SaveAsChapterTemplateModal({
  open,
  chapter,
  bookId,
  existingTemplate,
  ...
  const [name, setName] = useState(
    existingTemplate?.name ?? chapter?.title ?? "",
  );
  ...
  const handleSubmit = async () => {
    ...
    if (!trimmedName) {
      setNameError(
        t("ui.save_chapter_template.name_required", "Name ist erforderlich"),
      );
      return;
    }
    if (!trimmedDescription) {
      notify.error(
        t(
          "ui.save_chapter_template.description_required",
    ...
    try {
      if (isEdit && existingTemplate) {
        ...
        notify.success(
          t("ui.save_chapter_template.saved_edit", "Vorlage aktualisiert"),
        );
      } else {
        if (!chapter || !bookId) {
        ...
        notify.success(
          t("ui.save_chapter_template.saved", "Vorlage gespeichert"),
        );
      }
      resetForm();
      onClose();
      ...
      if (err instanceof ApiError && err.status === 409) {
        setNameError(
          t(
            "ui.save_chapter_template.name_taken",
        ...
        notify.error(
          err instanceof ApiError
            ? err.detail
            : t(
                "ui.save_chapter_template.save_failed",
          ...
          <div className={styles.body}>
            <div className="field">
              <label className="label">
                {t("ui.save_chapter_template.name", "Name")} *
              </label>
              <input
                className="input"
                value={name}
                ...
                placeholder={t(
                  "ui.save_chapter_template.name_placeholder",
                  "z.B. Interview-Vorlage",
                )}
              ...
              {nameError && (
                <div
                  ...
                  data-testid="save-chapter-template-name-error"
                >
                  {nameError}
                </div>
              )}
            ...
            <div className="field">
              <label className="label">
                {t("ui.save_chapter_template.description", "Beschreibung")} *
              </label>
              <EnhancedTextarea
                value={description}
                onChange={setDescription}
                placeholder={t(
                  "ui.save_chapter_template.description_placeholder",
                ...
                maxChars={500}
                ariaLabel={t(
                  "ui.save_chapter_template.description",
                  "Beschreibung",
                )}
              />
            </div>
            ...
            {!isEdit && (
              <div className="field">
                <label className="label">
                  {t("ui.save_chapter_template.content_mode", "Kapitelinhalt")}
                </label>
                <label className={styles.radioRow}>
                  ...
                  <div>
                    <div>
                      {t(
                        "ui.save_chapter_template.content_empty",
                        "Leerer Platzhalter",
                      )}
                    </div>
                    <div className={styles.hint}>
                      {t(
                        "ui.save_chapter_template.content_empty_hint",
                        "Empfohlen für wiederverwendbare Vorlagen",
                      )}
                    </div>
                  </div>
                </label>
                <label className={styles.radioRow}>
                  ...
                  <div>
                    <div>
                      {t(
                        "ui.save_chapter_template.content_preserve",
                        "Inhalt übernehmen",
                      )}
                    </div>
                    <div className={styles.hint}>
                      {t(
                        "ui.save_chapter_template.content_preserve_hint",
                        "Kopiert den aktuellen Kapitelinhalt in die Vorlage",
                      )}
                    </div>
                  </div>
                </label>
