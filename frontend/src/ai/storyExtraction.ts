...
/** Dispatch one completion by storage mode. Online uses the backend AI path
 *  (key on the server); offline calls the user's provider directly. */
async function complete(
  ...
  const user = messages
    ...
    .map((message) => message.content)
    .join("\n\n");
  const result = await api.ai.generate(user, system, bookId);
...
export async function applyStoryBible(
  ...
  for (const rel of extraction.relationships) {
    ...
    if (
      current.some(
        (existing) =>
          existing.target_entity_id === targetId && existing.relationship_type === rel.relType,
      )
    ) {
      continue;
    }
