...
- **Version:** 0.28.0 (Books dashboard reaches feature parity with Articles for bulk export: per-tile checkboxes + Select-all + sticky `BookBulkActionBar` + `POST /api/books/bulk-export` returning a date-stamped ZIP-of-individuals (epub/pdf/docx); 200-book hard limit; combined-multi-book mode intentionally NOT implemented per metadata-conflict rationale. Three new cross-platform installer entry points alongside `install.sh`: `install.command` (10-line macOS Finder wrapper), `install.ps1` (121-line PowerShell mirror generated from `install.ps1.template` via `make sync-versions`), `install.cmd` (7-line Windows batch wrapper invoking `install.ps1` with `-NoProfile -ExecutionPolicy Bypass`); all three ship unsigned per launch decision (Gatekeeper / SmartScreen warnings documented). `verify_version_pins.sh` rejects hardcoded version literals in the static wrappers. Launcher i18n extraction complete: ~50 hardcoded English dialogs (Docker-daemon retry, install-flow status sequence, update-available, uninstall + per-phase status, installation-moved + folder-picker, compose-failure, health-timeout, etc.) extracted into the JSON catalog with real DE umlauts; common-action strings (`OK`, `Cancel`, `Retry`, `Close`, `Open browser`) de-duplicated into a `common.*` namespace. `BIBLIOGON_DB_PATH` precedence flipped (DEP-DBPATH-01 step 2): `BIBLIOGON_DATA_DIR` now wins when both are set; legacy `DB_PATH`-only path keeps the v0.27.0 deprecation warning. Three new Playwright smoke specs for bulk article export. DEP-09 (Vite 7->8) + SEC-01 (vite-plugin-pwa CVE chain) unblocked upstream by `vite-plugin-pwa@1.3.0`; combined Vite 8 upgrade scheduled separately.)
