...
- **Version:** 0.26.6 (Foundation cleanup release. Filesystem isolation: production data now lives at `~/.local/share/bibliogon/` on Linux/macOS, `%LOCALAPPDATA%\bibliogon\` on Windows, `/app/data` in Docker — no longer in the project tree. Existing v0.25.0 installs auto-migrate on first start with a `.migrated-YYYY-MM-DD` breadcrumb. Lock-step versioning across all subsystems: only `backend/pyproject.toml` is hand-edited; `make sync-versions` propagates to frontend, launcher, plugins, install.sh template. CI release-gate workflow blocks artifact attachment on tag-vs-canonical mismatch. Pre-install stale-target safeguard in the launcher compares embedded `BIBLIOGON_TARGET_VERSION` against latest GitHub release before the welcome dialog. Docker upload persistence fix: uploads now live in the named `bibliogon-data` volume instead of the ephemeral `/app/uploads` COPY path. Backend `__version__` derives from pyproject via `tomllib`; frontend `__APP_VERSION__` from package.json via Vite define; launcher target version injected at PyInstaller build time. Tags v0.26.0..v0.26.5 are deprecated hotfix iterations preserved as historical record per `.claude/rules/lessons-learned.md` "Hotfix cluster tag policy".)
