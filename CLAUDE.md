...
- **Version:** 0.57.0 (The **God-Folder + browser-AI + desktop-update-checker** release. **Refactor:** the God-Folder campaign (#466) finished - ``frontend/src/{hooks,components,utils,services}`` are grouped into single-concern sub-packages (e.g. ``hooks/editor``, ``components/{book,articles,comics,story-bible,picture-book,export,settings,…}``, ``utils/{ai,format,icons,platform,editor}``, ``services/{audiobook,git,registries}``), the flat re-export **shims are removed** with importers rewritten, and a **directory-size ratchet CI guard** (Phase 5) blocks regrowth; shared primitives ``Badge``/``EmptyState``/``Tooltip`` promoted to ``lib/components``. **AI now runs browser-direct for all 6 providers in PWA/Dexie mode** - the wrong CORS gate is gone (OpenAI + Mistral are browser-capable, #468/#450), connection-test errors are classified honestly per failing layer (#446/#456), keys are stored **per-provider with an ``active_provider`` pointer** (#460) and shown in a Settings overview table with a **per-row Test button** (#459/#462), models load **dynamically per provider** with a preset fallback (#451), the default Anthropic model is a current one (not the retired ``claude-sonnet-4-20250514``, #454), and token/key fields no longer trigger the browser password manager (#448). **Desktop update-checker** (#477 Phases 1+2 / #479) - a GitHub-Releases version check for SW-less desktop builds: a startup auto-check hook, an update banner with release notes + dismiss-per-version, interval + dismissed-version logic, and a Settings **Updates** section. **Settings auto-save on change** (manual "Speichern" buttons removed, #473). **Client-side picture-book PDF** for offline/mobile (#497). **Comics:** set a panel image via **drag-and-drop** (#439). A **plugin-parity offline module seam** (#34) lands. Plus E2E flake stabilization (#436/#441/#484/#485) and test-isolation fixes. No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

**Unreleased on develop (toward v0.58.0):** **Desktop launcher → library-first** - the custom launcher is replaced by the ``docker-app-launcher`` PyPI package (now **0.12.1**: locale auto-detect, single-instance lock, rotating logging, platform-specific Docker diagnostics + guided start, a copy-log button, an always-available cleanup action, and an always-visible enabled/disabled button-state window relayout with tooltips); Bibliogon ships only ``launcher/launcher.json`` config + a thin three-responsibility wrapper, and deliberately omits the ``tray`` extra (it inflates the frozen binary ~6x) and leaves ``cleanup_search_paths`` empty (the cleanup scan would otherwise offer the live book-data dir for deletion). See ``docs/LAUNCHER-SPEC.md``. **KDP Publishing Wizard depth** - a **Format step** (eBook / Taschenbuch / Hardcover + KDP trim size + margin preset, #580) and a **KDP upload-guide step** (#581); the package endpoint now **renders the print PDF via WeasyPrint at the chosen trim size + margins** with crop/bleed marks (eBook → EPUB only; #583/#606). **First-install data-migration dialog** - a welcome dialog offers to import an online ``.bgb`` backup on first run (#591). **God-file WARN-tier top-5 split** (#207) - ``BookEditor`` / ``BookMetadataEditor`` / ``ArticleList`` / ``CollageCanvas`` / ``ChapterOutliner`` decomposed under 500 lines. **Optional per-chapter-collection colour** (#572). **Unified export metadata** - every export format now sources its metadata from one ``ExportDocument`` (SEO work; KDP-package metadata.json + Git-Sync frontmatter deferred). **Launcher help-docs** corrected to the real build-from-local-repo flow (no release download). Plus an **E2E flake sweep** (#436/#438/#440/#441/#442: fixed-timeout → deterministic waits, stale ``.json→.bgb`` specs, content-loading-overlay races, ``context.addInitScript`` for multi-tab onboarding suppression, ``retries:2``), an **E2E test-isolation audit** (data isolation already works; the residual flakes are infra accumulation in the single long-lived backend, not data leakage - ``docs/audits/e2e-isolation-audit-2026-06-22.md``), a **feature-screenshot catalog**, and a ``verify-theme`` fix. No schema migrations.
