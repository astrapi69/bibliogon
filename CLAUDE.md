...
- **Version:** 0.27.0 (Bulk article export + launcher first-run overhaul. Articles dashboard supports per-tile / per-row checkboxes, four-facet filter quartet (status / topic / series / tag — `series` and `tag` are new fields/filters), a sticky bulk-action bar, and a `POST /api/articles/bulk-export` endpoint that returns either a ZIP-of-individuals or one combined document for any of md / html / pdf / docx. Hard limit 200 articles per export; combined-Pandoc timeout 180s; fail-loud on unreachable images with the offending article's title surfaced. Article schema gains `Article.series` (flat free-string mirror of `Book.series`; idempotent migration `c0a1b2c3d4e5`). Launcher: every new user sees a non-skippable first-run welcome dialog explaining Docker prerequisites + a brief security trust statement linking to a new canonical Bibliogon Docker installation guide (`docs/help/{en,de}/install/docker-desktop.md`); UI is bilingual with a JSON i18n catalog (`launcher/bibliogon_launcher/locales/{en,de}.json`), OS-locale detection, and a Settings > Language picker. Docker-missing dialog moved from single-OK error to three-button choice. `BIBLIOGON_DB_PATH` deprecated with runtime warning when set without `BIBLIOGON_DATA_DIR` (step 1 of a 3-release deprecation cycle). Pre-push git hook enforces `pre-commit run --all-files` on tag pushes (`make install-hooks`); closes the v0.26.0..v0.26.6 hotfix-chain class. Frontend / backend version cross-check at app start logs a console.warn on mismatch (fails open). Installer discovery: D-05 closed as won't-fix per Docker EULA; D-06 (cross-platform installer scripts) and D-07 (winget + Homebrew) added.)
