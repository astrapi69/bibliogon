...
- **Version:** 0.23.0 (Plugin-git-sync full PGS-02..05 rollout: bi-directional Commit-to-Repo with ambient-cred push, three-way smart-merge with per-chapter conflict resolution UI, multi-language branch linking via main-XX detection + Book.translation_group_id, core-git bridge with unified-commit fan-out under per-book lock. Plus forwarded from v0.22.1: multi-book BGB import on XState v5, sticky-footer pattern across 13 dialog modals, EnhancedTextarea with css/markdown/html preview, WizardErrorBoundary with Copy details + Report Issue. Known limitation: PAT-via-UI deferred; push needs ambient SSH/credential helper. Migrations c6d7e8f9a0b1 (tts_speed backfill from v0.22.1), d7e8f9a0b1c2 (git_sync_mappings), e8f9a0b1c2d3 (translation_group_id) idempotent.)
