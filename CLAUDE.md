...
- **Version:** 0.30.0 (Launcher localized in 8 languages: en/de/el/es/fr/pt/tr/ja. The OS-locale resolver in `_current_lang()` was expanded from a hardcoded de-or-en branch to a prefix-matching loop over all 8 supported languages (de_DE / de_AT / de_CH all resolve to `de`, pt_BR and pt_PT both resolve to `pt`, etc.). Settings dialog gains six new native-name labels (Ελληνικά, Español, Français, 日本語, Português, Türkçe). Three of the six new catalogs (Greek, French, Spanish) are user-validated; the other three (Portuguese, Turkish, Japanese) ship with explicit `_meta.review_status: "pending native speaker"` blocks documented in `launcher/bibliogon_launcher/locales/REVIEW_STATUS.md` and machine-enforced by 31 new launcher i18n parity tests. User self-validation surfaced 4 register / idiom corrections (Greek `containers` → `κοντέινερ` and `φυλλομετρητής` → `πρόγραμμα περιήγησης`; French `Compris, continuer` → `J'ai compris`; Spanish `Internet` → `internet` per RAE 2014+). DEP-DBPATH-01 cycle closes: step 3 removes `BIBLIOGON_DB_PATH` as a path override; if still set in env, a single warning at startup names the ignored value but the variable has no effect. Priority chain collapses to TEST → DATABASE_URL → BIBLIOGON_DATA_DIR → platformdirs default. Five new bilingual core help pages: `developers/architecture.md`, `developers/contributing.md`, `developers/deployment.md`, `developers/api-reference.md`, `books/bulk-export.md`, `install/cross-platform-installers.md`. Plugin dev guide refreshed for Vite 8 (Rolldown bundler) + Node 24. Documentation drift sweep (README en+de versions, README-de.md ~50 ASCII transliterations converted to real umlauts, tester-onboarding Node version + DB-path migration, ux-conventions tab marker, mkdocs.yml orphan-page nav wired). Pre-v0.30.0 dependency currency sweep: fastapi 0.135 → 0.136 lock-step across 11 pyprojects, uvicorn 0.44 → 0.46, python-multipart 0.0.26 → 0.0.27, ruamel-yaml 0.18 → 0.19, plus in-range patches. Pre-v0.30.0 retrospective at `docs/journal/retrospective-pre-v0.30.0.md` covers the v0.27.0 → v0.30.0 period (4 calendar days, 4 tagged releases, 49 commits) with 5 concrete commitments for v0.30.0+.)
