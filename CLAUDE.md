...
- **Version:** 0.31.0 (Universal AI templates for Articles + Books with three first-class workflows: built-in AI, custom local endpoint via the new Settings "Custom" preset (LM Studio / Ollama), and external AI via YAML round-trip — the same `.biblio.yaml` artifact powers all three modes. Per-field-class selection via the new `FieldClassDialog` (cover prompt / SEO / tags / excerpt / chapter summaries / inline image prompts). Bulk operations with configurable per-batch caps (`ai.bulk.max_ai_fill` + `max_ai_template` in `backend/config/app.yaml`); live cost projection in the bulk AI-fill dock. Bulk-delete for Articles + Books with the 200-row cap removed (per-operation cost-profile rule: SQL bulk DELETE is sub-second); destructive permanent-delete behind a numeric type-to-confirm gate via new reusable `TypeToConfirmDialog`; soft-delete with `notify.bulkAction` Undo toast that restores every row. Medium HTML archive importer (10th plugin) producing Article + Publication + `ArticleImportSource` provenance rows per `posts/*.html`, with langdetect-based auto language detection, image-downloader-by-default for data sovereignty, first-body-image-as-featured toggle, dedicated `/articles/import/medium` page route. Medium comments routing via length+structure heuristic into a new `article_comments` table (three modes: as_comments / as_articles / skip; two orphan-handling modes: store / skip); editor sidebar `ArticleCommentsPanel`, dashboard + list-view comment-count badges, Settings comments-admin tab. Four new CSS theme tokens (`--surface-2`, `--danger-bg`, `--success`, `--warning`) defined across all 6 palette × dark-mode combos closing a silent fall-through-to-hex regression on 9 v0.30.x-shipped components. i18n catalogs es/fr/el/pt/tr/ja carry top-level `_meta.review_status` markers for the three new namespaces (`ai_template`, `bulk_ai_fill`, `comments`) shipping passthru-English; companion `backend/config/i18n/REVIEW_STATUS.md` documents the marker and PR-based correction workflow. Path-isolation defense-in-depth: `backup_history.json` + `plugins/installed/` resolve via `get_data_dir()` per the explicit "Filesystem isolation" rule; data_dir_migration auto-moves legacy locations on first boot of v0.31.0 for dev installs (production Docker was never affected). The `_base_dir / "config" / "app.yaml"` writes in settings.py deferred as `PROD-WRITES-ARCHITECTURE-01` P3. New A1 bulk-delete E2E smoke spec (`e2e/smoke/bulk-delete.spec.ts`) pins type-to-confirm gate + Undo restore + count=1 fallthrough. 157 commits since v0.30.0. Three pre-release audits at `docs/audits/pre-release-{verification,coverage,ux}-2026-05-12.md`.)
