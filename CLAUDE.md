...
- **Version:** 0.34.1 (Routine dependency refresh — pure lock-file refresh; no API/behavior changes. Backend deps: pip 26.0→26.1, numpy 2.4.4→2.4.5, pandas 3.0.2→3.0.3, requests 2.34.0→2.34.2, decorator 5.2→5.3, librt 0.9→0.11. Launcher: packaging 26.1→26.2, pyinstaller-hooks-contrib 2026.4→2026.5. Frontend: dompurify 3.4.3→3.4.4. All via `poetry update <allowlist>` discipline. Plugin pyprojects don't share any of the bumped packages directly — `make verify-plugin-locks` confirmed zero drift. Deferred majors: elevenlabs 0.2→2.47, mypy 1.20→2.1, weasyprint 66→68, TipTap 2→3 (DEP-02), @types/node 24→25, @vitejs/plugin-react 5→6 — each gets its own focused session. Pin-loosenings (uvicorn ^0.46→^0.47, python-multipart ^0.0.27→^0.0.29) deferred as a separate decision class from lock-file refresh.)

Previous release — **v0.34.0** (Picture-Book Phase 4 foundation + Books-only Categories/BISAC + Authors-Database completion + Medium-import v2 preview. Four large feature streams: (1) Picture-Book authoring — `book_type` discriminator on Book (`prose` | `picture_book`, immutable after create), new `pages` table + `/api/books/{id}/pages` CRUD, dashboard split-button "Create Picture-Book" affordance, three-pane PageEditor (thumbnails sidebar with @dnd-kit drag-reorder + canvas + properties pane), 5 distinct PageLayout variants (speech_bubble, image_top_text_bottom 70/30, image_left_text_right 60/40, image_full_text_overlay, text_only) each with dedicated CSS-Module classes, per-page `layout_config` JSON (renamed from speech_bubble_config) driving anchor/opacity/size + image-row positioning/fit/split-ratio, on-image hover overlay for image-replace, WeasyPrint ^66.0 dep + `picture_book_pdf` generator skeleton (route + UI lands v0.35.0). (2) Categories + BISAC: new `Book.categories` (KDP-aligned JSON list) + `Book.bisac_codes` (regex `^[A-Z]{3}[0-9]{6}$`), Marketing-tab wires both with shared `CategoryInput` + `BisacCodeInput`, KDP plugin metadata-checker extension, intentional Books-only asymmetry vs Articles' topic+tags. (3) Authors-Database: standalone `authors` table + `/api/authors` CRUD + Settings tab, slug auto-gen with German/Nordic diacritic transliteration + NFKD-fold + emoji fallback, Phase 2 Wizard datalist on Article-to-Book Step-1 with multi-article pre-fill + add-to-DB checkbox on submit (Books-only at conversion surface; editors get datalist follow-up). (4) Medium-import v2 preview-table: upload → preview checkbox table → uncheck-to-skip → import selection, per-post badges (Article/Comment/Duplicate/Warnings), Playwright smoke. Plus Comments trash-lifecycle (Bug 10 — closes v0.32.0 omission where 61 user-trashed comments were stuck in invisible purgatory; 5 new `/api/comments/trash/*` endpoints + view-mode toggle + per-row Restore/Permanent-Delete + Empty-Trash CTA + bulk-action bar in trash view), Categories+BISAC Marketing-tab leak hotfix (forceMount actively prevented Radix from applying `hidden` on non-active tabs), Picture-Book bugs A+C closed by Fix A purge layout_config on layout switch (Fix B namespace deferred to 4c-B), Bug B image_full_text_overlay default image_fit cover, 6 UX bug fixes from manual smoke (Bug 1 back-button history; Bug 3 independent trash view-mode defaults; Bug 4a-c Comments-Admin bulk-delete + preview modal + Reclassify-in-preview; Bug 6 menu items must not preventDefault on dialog-trigger across 6 callsites; Bug 7 mock-contract repair; Bug 11 BookDashboard list-view selection checkboxes for AD parity). 3 new lessons-learned rules (Half-wired feature lifecycle extended to frontend shape; Half-wired trash lifecycle canonical example; Test-isolation discipline — never run integration smoke outside pytest). Articles-vs-Books asymmetry tally promoted to 8 confirmed instances. ~100 commits since v0.33.0.)
