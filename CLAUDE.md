...
- **Version:** 0.60.0 (The **boot-resilience + PWA-update-flow + release-automation** release. **Fixed:** the data-dir migration no longer treats an **empty** pre-created target dir as a conflict - the class where any setup with legacy ``backend/plugins/installed`` content crash-looped at boot (RuntimeError in the lifespan, port never bound, every request answered 502 by the dev/prod proxy); the eager import-time mkdir is removed, ZIP-installed plugins load from the canonical data dir post-migration, and ``make dev`` aborts loudly when ``/api/health`` never comes up (#715). The **i18n boot race** that left the whole UI German when a straggling ``de`` catalog response overwrote the saved language's catalog (TC-052 nightly flake, real-user impact on slow networks) is closed with a stale-response guard (#714). The **client-side picture-book PDF** keeps colour + quality - in-cap images embed their original bytes untouched, genuine downscales composite on white instead of black (#692). **Added:** a deploy-independent PWA ``version.json`` build artifact + SW/GitHub-release **update-banner dedup** (#697), and an iOS-standalone **"close and reopen" update hint** (WKWebView activates a new SW only after a full app restart, #700). **Changed:** ``docker-app-launcher`` **^0.12.1 → ^0.28.0** - compose start force-recreates (no stale container after an update), one-step ``--update`` verb + GUI button, ``--doctor`` diagnostics, live log follow, cancellable operations with a cross-process guard, system light/dark following, translated failure causes (#702/#705/#709); **release orchestration** moves into Makefile targets (``release-prepare``/``release-finish``) + a one-click Gitflow workflow (#686/#688); TDD inner-loop targets + ``make pre-commit`` (#690/#693/#716); the web-speech nightly E2E stub fixed via ``defineProperty`` (#712). No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.59.0** (The **offline-authoring-depth + writing-insights + God-file burn-down** release. **Writing-Statistics dashboard** at ``/statistics`` - a today progress ring + a yesterday comparison, a last-7-days chart with the daily average as a reference line, per-book project progress with an estimated completion at the recent pace, and a contribution-style streak heatmap, all aggregated client-side so it works online + offline (#668). **Keyboard-shortcuts overview dialog** with search + context-awareness (#667). **Browser-native read-aloud (Text-to-Speech)** in the editor via the Web Speech API - capability-detected, runs **fully offline** with no backend, distinct from the multi-engine audiobook export which stays desktop-only (#666). **Offline AI grammar-check + translation** in the editor, run **browser-direct** against the configured AI provider and gated **disabled-until-a-key-is-set** (the ``ai-grammar`` / ``ai-translate`` features), while the server LanguageTool + DeepL/LMStudio paths stay desktop-only (#661/#669). **Client-side offline book templates** ("Aus Vorlage" in Dexie mode, #670). **Settings** gains a preview/test-version banner + build-info + a share-app QR section (#644). A **GH-Pages static preview deploy** (#639) + a static-smoke gate for the backendless build (#640/#641). **Changed:** the **God-file WARN-tier burn-down** decomposes ``ArticleEditor`` (sidebar extraction, #207/#634), ``GitSyncPage`` (#648), ``ConvertToBookWizard`` (#650), ``LayoutConfigImageRow`` (#652), ``useComicBookEditor`` (#654), ``AboutSettings`` (#676), ``CreateBookForm`` (#678), the ``platform.ts`` API client (#680), ``ComicBubble`` (#682) and ``CommentsAdminSection`` (#684) under 500 lines, with ``Editor.tsx`` whitelisted as the irreducible TipTap editor shell (#656); the ratchet baselines are renamed to the extension-less ``.dirsize-``/``.filesize-`` convention (#637/#646). **Fixed:** visual-regression-suite stabilization - onboarding dialogs suppressed + the flaky mobile-metadata case skipped (#628/#638). No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.58.0** (The **desktop-launcher library-first + KDP-publishing-depth + SEO** release. **Desktop launcher → library-first:** the hand-written launcher is replaced by the reusable ``docker-app-launcher`` PyPI package (``^0.12.1``); Bibliogon ships only ``launcher/launcher.json`` + a thin three-responsibility wrapper - a persistent single window, Docker-first check + guided start, install progress, port-conflict handling, a management dialog with reachable Stop/Uninstall, locale auto-detect, single-instance lock, and rotating logging; the ``tray`` extra is omitted (~6x frozen-binary inflation) and ``cleanup_search_paths`` left empty so the cleanup scan can't offer the live book-data dir (#519/#521/#588-#618, see ``docs/LAUNCHER-SPEC.md``). **KDP Publishing Wizard depth:** a **Format step** (eBook / Taschenbuch / Hardcover + KDP trim size + margin preset, #580) and a **KDP upload-guide step** (#581); the package endpoint renders the print PDF via WeasyPrint at the chosen trim size + margins with crop/bleed marks (eBook → EPUB only, #583/#606), and ``metadata.json`` gains ``isbn``/``format``/``trim_size``/``page_count`` (#612/#614). **SEO:** Open Graph / Twitter / JSON-LD meta tags for the PWA + the client-side export engine across every format (#605/#607), a YAML front-matter block on each Git-Sync chapter ``.md`` (#614), and a 1200×630 social ``og-image`` (#620). **First-install data-migration dialog** offers to import an online ``.bgb`` backup on first run (PWA → desktop, #591/#593). **Scrivener ``.scriv`` import** (the ``.scrivx`` index parsed with ``defusedxml``, #554). **Chapter Outliner depth:** per-chapter **synopsis** (auto-generate from the first paragraph) + **Inspector notes** + a project-notes field (#551/#567), and **manual chapter collections** with an optional per-collection colour (#553/#572). The diagnostic **event recorder** gains a structuring axis (category/action/appState, EVT-05) behind a single feature gate (EVT-06). An **automated feature-screenshot catalog** (36 on-demand capture specs, ``make capture-screenshots``, #556). **Changed:** the **God-file WARN-tier top-5 split** (#207) decomposes ``BookEditor``/``BookMetadataEditor``/``CollageCanvas``/``ArticleList``/``ChapterOutliner`` under 500 lines; offline **grammar + translation** are gated **disabled-with-reason** (no browser path, #34); launcher help-docs corrected to the build-from-local-repo flow (#603). No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.57.0** (The **God-Folder + browser-AI + desktop-update-checker** release. **Refactor:** the God-Folder campaign (#466) finished - ``frontend/src/{hooks,components,utils,services}`` are grouped into single-concern sub-packages (e.g. ``hooks/editor``, ``components/{book,articles,comics,story-bible,picture-book,export,settings,…}``, ``utils/{ai,format,icons,platform,editor}``, ``services/{audiobook,git,registries}``), the flat re-export **shims are removed** with importers rewritten, and a **directory-size ratchet CI guard** (Phase 5) blocks regrowth; shared primitives ``Badge``/``EmptyState``/``Tooltip`` promoted to ``lib/components``. **AI now runs browser-direct for all 6 providers in PWA/Dexie mode** - the wrong CORS gate is gone (OpenAI + Mistral are browser-capable, #468/#450), connection-test errors are classified honestly per failing layer (#446/#456), keys are stored **per-provider with an ``active_provider`` pointer** (#460) and shown in a Settings overview table with a **per-row Test button** (#459/#462), models load **dynamically per provider** with a preset fallback (#451), the default Anthropic model is a current one (not the retired ``claude-sonnet-4-20250514``, #454), and token/key fields no longer trigger the browser password manager (#448). **Desktop update-checker** (#477 Phases 1+2 / #479) - a GitHub-Releases version check for SW-less desktop builds: a startup auto-check hook, an update banner with release notes + dismiss-per-version, interval + dismissed-version logic, and a Settings **Updates** section. **Settings auto-save on change** (manual "Speichern" buttons removed, #473). **Client-side picture-book PDF** for offline/mobile (#497). **Comics:** set a panel image via **drag-and-drop** (#439). A **plugin-parity offline module seam** (#34) lands. Plus E2E flake stabilization (#436/#441/#484/#485) and test-isolation fixes. No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.56.0** (The **bulk-delete-fix + publish catch-up** release - the first published release since v0.51.0, re-syncing the GitHub releases stream with the v0.52.0->v0.55.0 tags (cut but never published). **Fix:** bulk-delete no longer silently fails above 200 selected items - the frontend delete handlers had reused the export cost-profile cap (200); delete is uncapped (a DB UPDATE/DELETE per row), so the cap is removed for both Articles and Books (#417). **Added:** Settings previews the event-log + image-cache contents before clearing them (#408). **Changed:** downloads keep the native ``downloadBlob`` path (file-saver dropped, hardened + de-duplicated, #416); the ASCII filename-slug consolidates into one backend helper (#390); Phase-1 multi-viewport visual-regression baselines + nightly CI (#411). **Fixed:** quality-report chapters order by sidebar matter groups, not raw position (#415); custom-combobox commit-on-outside-close (#404); mobile editor sidebar click-to-close overlay across all four overlay-sidebar editors (#405/#406). No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.55.0** (The **Git/Import-depth + quality-report + mobile-polish** release. File / **GitHub** / **URL** import converge into one tabbed dialog (HTML ``.html``/``.htm`` accepted, #352/#361); the book metadata gains a **Git-Repository** panel (URL + remote default branch + sync status + **Pull**, #358/#359/#363), and **Git-Sync now fetches before diffing** so chapters pushed upstream are detected as new/changed instead of staying invisible (#378). The **Story Bible** can be AI-generated from a book's text; the right-click **editor context menu** exposes all toolbar functions (shared ``EditorMenu`` across all editors); **Settings > About** gains a "Check for new version" button (#367). The **quality-report PDF** mirrors the on-screen layout and numbers the chapter table by logical book order (#356/#360); chapter metrics compute client-side offline (#396). The **app icon** is redrawn as a clean SVG with a full favicon/PWA set (16/32/180/192/512 + maskable, #399). **Fixes:** metadata-Pull guard when no local clone (#375), Gemini 400 as invalid-API-key (#357), mobile/responsive (#394/#398/#401). No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.54.0** (The **baseline-zero + CI-tiering + authoring-depth** release. The god-file burn-down hits its goal - ``.filesize-baseline`` is **0** (``Editor.tsx`` 981, ``dexie-storage.ts`` 13). CI splits into **three tiers** (fast ~6-min PR gate via ``vitest --changed`` / ``pytest --testmon``, full nightly, weekly security) and **Security-CI Phase 2** blocks on Critical/High; the **complexity baseline** tightens 20->15; a backend coverage audit records 90.1%. The **quality report** gains nested-sentence (Schachtelsatz) candidates, a sortable chapter comparison table, a genre benchmark for Flesch, and a scope disclaimer. Authoring adds **selective data export**, a **book-language combobox**, an **HTML export preview**, and drag-and-drop file import. No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.53.0** (The **god-file burn-down + offline-depth** release. A backend service-extraction series (16 splits) + a frontend burn-down cut ``api/client.ts`` 5212->13 and ``.filesize-baseline`` 16->5; the generic per-``(table,id)`` **serialized write-queue** now covers every read-modify-write storage method, closing the settings-clobber class. **Offline article featured-image assets** (#157) store image bytes in IndexedDB (as ``ArrayBuffer``) + a service-worker ``/api/...`` intercept + ``useArticleImageUrl`` resolver. Adds **selective data export**, the cross-project **book-metadata section-nav** Settings-sidebar pattern, and a **book-language combobox**; an E2E smoke workflow + weekly dependency-security watcher join CI; White-Label is removed; a manual test plan + release checklist are written down. v0.53.1 moved the quality-report download buttons to the top (#257). No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.52.0** (The **offline-depth + stability-hardening** release. The offline import wizard now also restores **``.bgb`` full-data backups** entirely client-side (browser format detection, no ``/api``), and the **Medium import** moves to a dedicated page with per-post progress + first-image thumbnails. Author **pen names** become a proper ``<select>`` (no more free-text datalist); the **view-switcher + responsive sidebars** land across more surfaces; the **Article / picture-book editors** wrap their header toolbars on narrow viewports. Under the hood the **storage seam is hardened** - nine direct-seam bypasses removed with a ``settingsSeamGuard`` enforcement test, and all read-modify-write methods route through a single **serialized write-queue** (``serializedUpdate``), closing a settings-clobber data-loss path. Two CI watchers join the gate (**cohesion** file-size + **complexity** radon/ruff-C901/ESLint, both warn-only), the **Vibe-Coding policy** + **MODULE-ARCHITECTURE** reference are written down, and a long tail of **E2E smoke flakes** is stabilized. Six systematic **LaTeX export** bugs are fixed. No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.51.0** (The **offline-feature-strategy + import-wizard + About-depth** release, 36 commits since v0.50.0. Offline feature-gating is unified behind a central **feature registry** (``@astrapi69/feature-strategy``, ``useFeature(id)`` via ``AppFeatureProvider``): every gated surface resolves to **active**, **disabled-with-reason**, or **hidden** under policy #78 - *nothing the user owns is hidden; it is active or disabled with an explanation*. The whole desktop-only set (Git sync/backup, TTS audiobook, Pandoc PDF export, LAN mode, version history, backup compare/history, book-templates, ``ai-template-file-io``) moves onto the registry, so the backendless GitHub-Pages PWA explains *why* a feature is unavailable instead of silently dropping it (#63, #66, #67, #78). The **import wizard works fully client-side offline**: a browser format detector (``detectFormat``) + router (``importFile``) route Markdown/Text/HTML into a new book+chapter, JSON full-data backups through the restore path, and Medium ZIPs through the client importer - all via the storage seam with zero ``/api`` calls; only ``.bgb`` shows the desktop-app hint (#76, #82). The **About page** gains the build **branch**, a client-side **System** section (storage mode + ``navigator.userAgent`` platform), **Contributors/Built-with/AI-assistance**, donation links, and a **License & Resources** section - all rendering offline (#84, #87). **LaTeX (.tex)** becomes selectable in the export UI in both modes via the client engine (no Pandoc gate) (#89). Development adopts a **gitflow** model: ``develop`` = active development, ``main`` = releases only (#79). **Fixes:** editor template-insert error reporting (#70), Danger-Zone backup button moved onto the Settings page (#72), an undefined ``--surface`` theme-token, and E2E smoke stability (#85, #86, #92). No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.50.0** (The **full-data backup + testing-infrastructure** release, 25 commits since v0.49.0. A single **JSON backup bundle** exports/imports the entire workspace - books + chapters (with content), articles, authors, settings, story-bible entities - through the ``IStorageService`` seam, so it works identically online and offline; the **BACKUP-AKZEPTANZTEST** E2E gate exercises the full export -> reset -> import -> verify cycle (it caught a real export bug on its first run). The **Danger-Zone reset** gains a backup-before-reset dialog. Two automated quality gates land: **Playwright visual regression** (36 screenshot baselines, 3 views x 12 theme variants) and **axe-core a11y** checks (5 routes) - the latter surfaced a WCAG-AA contrast bug fixed across 7 of 12 theme variants. The storage seam is tightened to **strict dual-storage** (zero direct ``api.*`` CRUD from components), Settings lists **auto-save**, the authors DB gets **JSON export/import**, and Python **bandit** SAST + **madge** circular-dependency detection join CI. **Fixes:** create-article ``?type=`` deep-link race (#65), smoke-gate hardening (#64). No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)

Previous release — **v0.49.0** (The **editor v3 + offline depth + quality-infra** release, 38 commits since v0.48.0. The TipTap editor moves to **v3** (prosemirror-search adapter) with **node-based math** (KaTeX, inline ``$...$`` + block ``$$...$$``, input/paste rules, Formel toolbar button); **LaTeX (.tex)** joins the client-side export formats. More surfaces go fully offline: **Writing History** is aggregated client-side from the ``writingSessions`` Dexie table (summary/streaks/per-book/per-chapter), **book trash** soft-deletes + restores through the ``IStorageService`` seam, and the **Danger-Zone reset** (Dexie wipe+reseed) + **author profiles** work backendless. The **BookEditor sidebar** tools fold into a viewport-responsive **Werkzeuge** group (Radix Collapsible). **Authors-DB:** ``is_profile_author`` flag + Profile->DB sync + Profile badge. **Comics:** per-panel image upload. **Quality infra:** coverage baseline (backend 89.6% / frontend 72.3%) + ``coverage-backend``/``coverage-frontend`` targets, dependency security scanning (``npm audit`` + ``pip-audit``) in CI + ``make audit*``, **ESLint flat config + Prettier**. **Fixes:** autosave 409 self-conflict (authoritative version ref), chapter-click switches editor content, draft recovery on the v3 instance, icon-only-button aria-labels (axe button-name), consistent bulk-action-bar button classes. No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.)
...
- **Backend:** Python 3.12+, FastAPI, SQLAlchemy 2.0, SQLite, Pydantic v2, Poetry
...
- **Desktop launcher:** docker-app-launcher ^0.28.0 (PyPI), config in `launcher/launcher.json` + thin wrapper (see docs/LAUNCHER-SPEC.md)
...
| plugin-learnset    | core    | export     | Export a book as an adaptive-learner learn set: schema-validated scaffold ZIP (`GET /api/learnset/{id}/export`). Desktop-only by design (#763/#775): the ZIP is assembled and validated server-side. |
| plugin-promotion   | core    | -          | Portfolio board of retail formats (eBook/paperback/hardcover) per book: format status, store URLs, ASIN, universal link, CSV import with dry-run (`/api/promotion`) |
...
| plugin-aplus       | core    | -          | Amazon A+ Content package (short description, 3 bullets, header + three-image modules with image prompts) generated by AI from the book's metadata, validated against a versioned per-language ruleset (`bibliogon_aplus/rules/ruleset.yaml`), cached per book + language in `aplus_content`. UI: the A+ Content section of the book metadata editor (#887): an editable document per book + language (`aplus_documents`, storage seam, works offline) built from module templates; AI fill desktop-only (#891). |
