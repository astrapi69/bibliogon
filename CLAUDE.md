...
- **Version:** 0.25.0 (Articles reach full parity with books — dashboard chrome, soft-delete + trash, AI-generated SEO, backup manifest 2.0 with articles/publications/assets, CIO Import Wizard restores articles. Three-layer secrets config (project YAML < user override at `~/.config/bibliogon/secrets.yaml` < `BIBLIOGON_*` env-vars). Donations S-01 Settings tab "Unterstützen" with four-channel grid (Liberapay, GitHub Sponsors, Ko-fi, PayPal). T-01 inline-styles refactor: 22 components/pages migrated to CSS Modules, ~700 inline-style call-sites eliminated. Settings mobile hamburger F-01. `make dev-bg` robustness F-02 with setsid + log capture. Centralised `app.__version__` killed a stale `0.15.0` hardcode in `/api/health`.)
