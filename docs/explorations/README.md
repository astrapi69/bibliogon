...
| [editor-menu-model.md](editor-menu-model.md) | design done / 0 built | **Offen** — model designed, validated against all four shipped `EditorMenu` builders, and all twelve design decisions settled (#706). Scope decided as editor menus only; settings + primary nav explicitly out. No package. Two spin-off test findings filed separately; the focus state machine is a recorded refusal (9 of 91 code lines are pure logic). | A third repo needs the shape, OR the coordinated renderer+builders PR is scheduled anyway. Adoption requires renderer + all four builders in ONE PR — three failure modes are silent. |
