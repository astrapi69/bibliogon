...
| [editor-menu-model.md](editor-menu-model.md) | design done / 0 built | **Offen** — model designed and validated against all four shipped `EditorMenu` builders; scope decided as editor menus only (settings + primary nav explicitly out). No package. Two spin-off test findings filed separately; the focus state machine is a recorded refusal (9 of 91 code lines are pure logic). | A third repo needs the shape, OR the coordinated renderer+builders PR is scheduled anyway. Twelve open design decisions first. |
