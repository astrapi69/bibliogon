...
with a Git repository get materially better, the file / GitHub / URL import
paths converge into one dialog, the quality report's PDF finally mirrors the
on-screen layout, the Story Bible can be generated by AI from the book text,
and a sweep of mobile/responsive fixes lands alongside a redrawn book-icon
set. No schema migrations; existing data + ``.bgb``/``.bgp`` unaffected.

### Added
- **Unified import dialog** — file import (Markdown / Text / **HTML .html/.htm**),
  **GitHub-repo import**, and **URL import** now live in one tabbed dialog
  (#352, #361), with edge-case + boundary test coverage for the GitHub/URL
  paths (#364).
- **Git-Repository metadata field** — the book metadata gains a panel showing
  the repository URL, the active/remote **default branch**, sync status, and a
  **Pull ("Aktualisieren")** button; the existing repository URL is also
  surfaced on the Git-Sicherung page (#358, #359, #363).
- **Story Bible from book text** — AI-generate Story Bible entities + a
  Storyboard directly from a book's chapters.
- **Editor context menu** — the right-click menu exposes all toolbar
  functions, grouped; the generic ``EditorMenu`` is reused across the Article,
  Comic, and Picture-book editors.
- **Settings → About** — a "Check for new version" button that runs an active
  service-worker update check (#367).
- **New app-icon set** — the open-book mark redrawn as a clean SVG with a full
  favicon/PWA set (16/32 tab, 180 apple-touch, 192/512 + maskable).

### Changed
- **Git-Sync detects new remote chapters** — "Auf Änderungen vom Repo prüfen"
  now fetches the remote (credential-optional for public repos) and
  fast-forwards before diffing, so chapters pushed upstream since import are
  detected as new/changed instead of staying invisible; a genuinely
  unreachable remote surfaces a clear error.
- **Quality-report PDF** mirrors the on-screen layout (colored tables, Flesch
  scale, thresholds) and strips HTML from nested-sentence candidates
  (#356, #360); the chapter table is numbered by logical book order rather
  than raw position.
- **Offline quality metrics** — chapter metrics are computed client-side in
  Dexie/offline mode (#396).
- **Config writes** — app-config overlay writes are atomic and route through a
  serialized read-modify-write queue, closing a settings-clobber path (#351).
- **Docs / infra** — a Library-First audit + a 4-stage dependency-hierarchy
  principle; explorations cleanup (three shipped explorations archived,
  ``dependency-strategy`` refreshed, ``children-book-plugin`` marked shipped);
  ROADMAP + backlog reconciled to the post-v0.54.0 state.

### Fixed
- **Metadata Pull guard** — the Git-Repository Pull button is disabled when a
  repo URL is set but no local clone exists, instead of erroring with a 409
  (#375).
- **Gemini auth** — HTTP 400 auth failures are reported as invalid-API-key
  errors rather than "invalid request" (#357).
- **Mobile / responsive** — the Authors-Database toolbar wraps on narrow
  viewports (#401), the chapter sidebar is fully scrollable (#398), the brand
  text is hidden on mobile, and the recent-docs strip no longer x-scrolls the
  page (#394).
- **i18n** — the About up-to-date message uses the correct superlative (#369);
  the import hint lists ``.html``/``.htm``; a French settings-title accent.
...
feature-gating is unified behind a central **feature registry**
(`@astrapi69/feature-strategy`): every gated surface now resolves to one of
three states — **active**, **disabled with a reason**, or **hidden** — under
policy #78 *"nothing the user owns is hidden; it is active or disabled with an
explanation."* The whole desktop-only set (Git sync/backup, TTS audiobook,
Pandoc/LaTeX-PDF export, LAN mode, version history, backup compare/history, …)
moves onto the registry, so the backendless GitHub-Pages PWA shows *why* a
feature is unavailable instead of silently dropping it. The **import wizard now
works fully client-side offline**: a browser format detector routes Markdown /
Text / HTML files into a new book + chapter, JSON full-data backups through the
restore path, and Medium ZIPs through the client importer — all via the storage
seam with zero `/api` calls; only `.bgb` shows the desktop-app hint. The
**About page** gains the build **branch**, a client-side **System** section
(storage mode + platform from the browser), **Contributors / Built-with /
AI-assistance**, donation links and a **License & Resources** section — all
rendering offline. **LaTeX (`.tex`)** becomes selectable in the export UI in
both modes, generated by the client engine. Development adopts a **gitflow**
model (`develop` = active development, `main` = releases only). No schema
migrations; existing data, backups (`.bgb`) and projects (`.bgp`) are
unaffected.

### Added
- **Feature-strategy foundation**: a central feature registry + conditional
  strategy + `AppFeatureProvider`, with a reactive AI-key signal, consumed via
  `useFeature(id)`. (#63)
- **Three-state offline visibility** (active / disabled-with-reason / hidden)
  replacing the old hide-only gate; disabled controls carry a translated reason
  and whole disabled sections render a `FeatureNotice` card. (#66, #78)
- **Client-side offline import wizard**: in-browser format detection
  (`detectFormat`) + router (`importFile`) for Markdown / Text / HTML → new
  book+chapter, JSON full-data backup, and Medium ZIP; `.bgb` shows the
  desktop-app hint. (#76)
- **About > Branch**: the build's branch (`VITE_BUILD_BRANCH` / `GITHUB_REF_NAME`
  / git) alongside version, build hash and build date. (#84)
- **About page expansion**: a client-side **System** section (storage mode,
  data location, platform parsed from `navigator.userAgent`), **Contributors**
  (author, a Built-with toolchain tag list, AI-assistance credit), donation
  links, and a **License & Resources** section — all offline-capable. (#87)
- **LaTeX (`.tex`) export** is now selectable in the export UI; it runs through
  the client engine (no backend `.tex` exporter, no Pandoc gate) in both API and
  offline modes. (#89)
- **Danger-Zone backup button** moved onto the Settings page (out of the dialog
  flow). (#72)

### Changed
- **Gitflow**: `develop` is the active-development branch (GitHub default);
  `main` holds releases only. Feature/fix branches PR against `develop`; only a
  release merges to `main`. (#79)
- Offline feature gates migrated to the registry / `useStorageMode`: audiobook
  (TTS), backend Pandoc export engine, chapter snapshots (version history),
  AI generate/fill, Git sync/backup, LAN mode, translation links, writing-history
  CSV, book templates, backup compare/history, `ai-template-file-io`. (#63, #67)
- About's Credits / license / system rows are no longer backend-gated — they
  render client-side on the offline PWA.
- Export-page hint clarified: LaTeX source exports in-browser; only the
  high-fidelity Pandoc PDF requires the desktop app. (#89)

### Fixed
- Article-list import opened the backend wizard offline (`/api/import/detect`
  503); it now routes through the offline import dialog like the Dashboard. (#82)
- Editor template-insert errors are now passed to `notify.error` for an
  actionable toast. (#70)
- About built-with chips referenced an undefined `--surface` token (CI theme
  gate); pointed at the defined `--bg-card`.
- E2E smoke stability: offline-PWA markdown-import strict locator, copy-toolbar
  chevron-menu lost-click under load, create-article `?type=` read-after-write
  race, and the about-dialog section testids after the About expansion.
  (#85, #86, #92)
