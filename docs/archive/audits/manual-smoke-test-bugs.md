...
| 11 | Session 4c-A: 4 layout-config bugs (A typography poor / B overlay text wider than image / C image_fit crops / D scope-add width+height) | Bug B Closed (`30edf35`); Bug C awaiting user new-page verification; Bug A + Bug D strategically re-framed as Tier-Property scope-add deferred to 4c-B | 2026-05-17 (`30edf35` Bug B fix; Bug A + D deferred to 4c-B; Bug C conditional on user verification) | PageCanvas image_full_text_overlay / image_top_text_bottom / image_left_text_right | "Default-Change-without-Migration" (first-instance incident — not formalized; if 2nd instance surfaces, file as lessons-learned class) | PICTURE-BOOK-TEXT-CONFIGURATION-01 (P3, captures Bug A + Bug D scope) |
...
| Half-wired feature lifecycle (state-write without inverse-mutation OR state-consumer) | 3 (Bug 10 Comments-Trash — backend shape, closed by Bug 10 fix; PB-PHASE4 Session 4 Layout-Picker without renderer — frontend shape, closed by Session 4 Commits 1-3; PB-PHASE4 `layout_config` field without UI — frontend shape **fully closed** by Session 4c read-path + write-path; this 3rd instance is the canonical complete-cycle example with narrower drag-to-position follow-up tracked as `PICTURE-BOOK-SPEECH-BUBBLE-DRAG-POSITION-01` P5) | Yes | `lessons-learned.md` § "Half-wired feature lifecycle" |
...
| ~~PICTURE-BOOK-SPEECH-BUBBLE-POSITIONING-01~~ | ~~P3~~ | ~~PB-PHASE4 Session 4 Pre-Inspection audit (filed 2026-05-17)~~ | ✅ **CLOSED** by Session 4c preset write-path (5 anchor presets + opacity + size). Narrower drag-position scope moved to `PICTURE-BOOK-SPEECH-BUBBLE-DRAG-POSITION-01` P5. | — |
| PICTURE-BOOK-SPEECH-BUBBLE-DRAG-POSITION-01 | P5 | PB-PHASE4 Session 4c close (filed 2026-05-17) | Drag-to-position UI for the speech-bubble (free-form positioning beyond the 5 anchor presets). Persists `{x_pct, y_pct}` dict extending the current preset-string shape. | User requests bubble positioning beyond the 5 presets OR drag-to-position UX feedback from a real picture-book authoring session |
| PICTURE-BOOK-SPEECH-BUBBLE-EXTENDED-PROPERTIES-01 | P3 | PB-PHASE4 Session 4c close (filed 2026-05-17) | Tier 1 + 2 extended bubble properties (11 props: visual style + typography) plus `bubble_width` + `bubble_height` replacement of the single `size` slider. Collapsible-sections UI pattern. | Scheduled 4c-B session OR user-feedback that the 3-property MVP is insufficient |
| PICTURE-BOOK-SPEECH-BUBBLE-EXTENDED-SHAPE-01 | P3 | PB-PHASE4 Session 4c close (filed 2026-05-17) | Tier 3 extended bubble properties (shape variants oval/rectangle/cloud/explosion + tail configuration + padding + font-style). | Comic-Plugin work starts (shape overlaps with comic-book bubbles) OR user requests bubble-shape variants beyond oval default |
| PICTURE-BOOK-TEXT-CONFIGURATION-01 | P3 | Session 4c-A manual smoke (filed 2026-05-17 — Bug A + Bug D strategic re-frame as sibling to bubble extended-properties) | Tier 1 + 2 text-configuration properties across image-based layouts (image_left_text_right + image_top_text_bottom + image_full_text_overlay typography + visual-style + sizing). Same Tier-Property-Pattern as bubble extended-properties — reusable `<CollapsibleSection>` + possibly `<TierPropertiesEditor>` extracted once for both. | Scheduled 4c-B "Tier-Property Pattern Application across Bubble + Text" session OR user-feedback that typography defaults are still poor after 4c-A push |
