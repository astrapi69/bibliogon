...
- Ziel: 5 Roundtrip-Tests für den kompletten Import/Export-Zyklus
- Ergebnis:
  - 5 Tests erstellt: project import/export, asset roundtrip, chapter type preservation, backup/restore, plain markdown import
  - Problem: Plugin-Export-Routen nicht in TestClient verfügbar (kein lifespan) - gelöst durch direkten scaffold_project()-Aufruf
  - Backend-Tests: 33 -> 38
...
- Optimierter Prompt: "Konvertiere backend/Dockerfile zu einem Multi-Stage Build: Builder-Stage installiert Poetry und resolved Dependencies, Runtime-Stage enthält nur Python, Pandoc und Produktions-Pakete. Füge .dockerignore hinzu."
...
- Ergebnis:
  ...
  - Build-Context auf Repo-Root geändert (nötig für Plugin-Path-Dependencies in pyproject.toml)
  ...
  - Problem: Plugins wurden als .pth-Links installiert (zeigen auf /build/ im Builder) - gelöst durch pip install --force-reinstall --no-deps
  - Verifiziert: Alle Plugins, uvicorn, FastAPI funktionieren im Runtime-Image
  - Image-Größe: 916MB (Pandoc allein ~200MB, Python site-packages ~370MB)
- Commit: c32f553
