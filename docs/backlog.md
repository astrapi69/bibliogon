...
Last updated: 2026-05-23 (RECURRING-COMPONENT-AUDIT-01 candidate #4 CLOSED via path α. AuthorSelectInput extracted in 1-commit a213788 — pure-presentational input + datalist + Add-to-Authors-DB checkbox; 2-site migration (CreateBookModal + ConvertToBookWizard). Pre-Coding-Reality-Check surfaced a NEW Pattern-B candidate (inline AuthorSelect at ArticleEditor.tsx:1216 + AuthorSelectField at BookMetadataEditor.tsx:542; estimated score 13) the original audit missed because file-level scan doesn't see inline functions inside large files; methodology gap documented in audit footnote with future-audit grep recipe. AUTHOR-DATALIST-EXTEND-EDITORS-01 deferred as UX-adjudication-required (the backlog item proposes replacing Pattern B with Pattern A at editor surfaces — design decision, not mechanical migration). Frontend Vitest 1792 → 1803 (+11: exactly the 11 new AuthorSelectInput cases); CreateBookModal 32/32 + ConvertToBookWizard 30/30 preserved without rewrites; backend baseline 2125 unchanged. Audit candidates remaining: #1 useSelection<T>() (deferred-design-intent) + #3 ListRow (score 13) + NEW Pattern-B AuthorSelect/AuthorSelectField (score 13). Prior 2026-05-22 close: RECURRING-COMPONENT-AUDIT-01 candidate #2 BulkActionBar via path γ pivot (c2305e7).)
