...
Last updated: 2026-05-21 (PLUGIN-COMICS-PHASE-2-PANEL-CONFIG-01 CLOSED. LayoutConfigComicPanel + panel-image upload + assetUrls Half-Wired closure shipped in 7 commits d3410fb..9062633. Closes Findings #1 (Panel-Image-Upload) + #3 (LayoutConfigComicPanel) from the 4-user-findings audit; ALL 4 findings now resolved. Tier1Section RCU 3rd-site application; option-A all-8-fields (user-adjudicated); panel_config + image_asset_id PATCH surface end-to-end. Frontend Vitest 1768 → 1783 (+15: C1+7 + C3+6 + C4+2); LayoutConfigComicPanel 13; ComicBookEditor.test 14 → 16; backend baseline 2125 unchanged (no backend migration); 1 new Playwright smoke (panel-image-upload round-trip with bbox dimension assertions per "Playwright-visible ≠ User-visible" LL); i18n × 8 (DE umlauts + EN + 6 passthrough-EN); orphan side_pane_panel_selected cleaned. Phase 3 (EXTENDED-FEATURES-01) trigger now fully met (BLOCKED-BY-Phase-2 lifted); ready as multi-session arc when prioritization elevates from P3 to P2. Prior 2026-05-20 closes: PLUGIN-COMICS-PHASE-1-MULTI-PANEL-LAYOUTS-01 (Findings #2 + #4) + PLUGINFORGE-RECURSION-LIMIT-REGRESSION-01 + PLUGIN-COMICS-SESSION-3-PAGES-CRUD-01 + Add-Panel diagnostic close. Parallel-session 2026-05-21: pluginforge ^0.8.0 → ^0.9.0 bump (0c966e0) shipped orthogonal to Phase 2; missing_target_application FilterReason i18n key added.)
