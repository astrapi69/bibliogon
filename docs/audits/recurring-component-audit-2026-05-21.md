...
> 6. **NEW candidate: Pattern B `AuthorSelect` / `AuthorSelectField` profile-select cluster — score 13 — SHIPPED 2026-05-23 (commit `11198a4`).** Two inline functions at [ArticleEditor.tsx:1216-1275](../../frontend/src/pages/ArticleEditor.tsx#L1216-L1275) (function `AuthorSelect`) and [BookMetadataEditor.tsx:542](../../frontend/src/components/BookMetadataEditor.tsx#L542) (function `AuthorSelectField`). The ArticleEditor inline doc-comment self-identifies as: *"Mirrors the BookEditor AuthorPicker's matched-state ProfileSelect, simplified for the Article editor."* Both use `<select>` with `<optgroup>` of user's-own profile (real-name + pen-names from `useAuthorProfile()` hook); closed list, no free-text. A1=2, A2=4 (explicit mirrors comment + drift potential), A3=4 (clean prop-injection: value + profile + onChange + optgroup labels), A4=3 → total **13**. Extracted as `frontend/src/components/AuthorProfileSelect.tsx` (~170 LOC + 12 Vitest cases). ArticleEditor inline function DELETED; BookMetadataEditor inline `AuthorSelectField` kept as thin adapter wrapping AuthorProfileSelect (the wrapping `<div className="field">` + label + manage-link stays per-site). Full Vitest 1803 → 1815.
>
...
>
> **Closure-in-action 2026-05-23 (commit `11198a4`):** Pattern B extraction shipped resolves the methodology-gap concretely. The gap-recipe above is now load-bearing for the NEXT audit iteration; this iteration's documented-in-place footnote serves as the audit-trail. Filing of a formal Lessons-Learned entry deferred per "single instance is incident, not pattern" discipline — Pattern B is the first known instance of the inline-function audit gap. If a third audit iteration ever surfaces another inline-function-missed-by-scan candidate, formalize as `.claude/rules/lessons-learned.md` "Audit methodology: grep inline functions inside large files, not just file-level components".
