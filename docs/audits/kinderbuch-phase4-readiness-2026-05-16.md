...
## Schema correction (2026-05-16, mid-session, supersedes earlier amendment)

After several iterations the user settled on the FLAT
discriminator scheme. This supersedes any prior "Visual-Books
umbrella + sub_type" wording in this audit; the body now reads:

- §1 line "Data model" — table cell:
  - **Before:** `Book.book_type ∈ {"prose", "children_book"}`
  - **After:** `Book.book_type ∈ {"prose", "picture_book", "comic_book"}`
    — flat. No `visual_sub_type` column.
- §4 "Open decisions" item 2 — Pydantic `Literal` type:
  - `Literal["prose", "picture_book", "comic_book"]` for `book_type`
- §4 item 4 — immutability rule applies to the one `book_type`
  column (PATCH attempting to change it returns 400).
- §5 "Session 2 scope" — Alembic migration adds the single
  `book_type` column; the Page model and the Pydantic schemas
  reflect that; Pages CRUD gates on
  `book.book_type == "picture_book"` (rejects prose AND
  comic_book).

Each visual book_type is owned by its own plugin:
- `picture_book` → `plugin-kinderbuch` (v1 active).
- `comic_book` → future `plugin-comics`. The value is reserved
  in the schema layer so the comics plugin can ship its own
  `panels` and `speech_bubbles` migration WITHOUT re-migrating
  the `book_type` column.

Reasoning for dropping the umbrella: an intermediate
`visual_book` value forces every "find the plugin for this
book" lookup to do TWO column reads (book_type + visual_sub_type)
and forces the schema to choose a Cartesian-product validator
(prose-with-sub-type and visual-without-sub-type both invalid).
With the flat scheme there is exactly one column to inspect
and one validator path.

Out of Session 2 (future plugin-comics, separate plugin):
- `panels` table.
...
with the §1 + §4 + §5 corrections in mind.
