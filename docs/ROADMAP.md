...
- [ ] **PGS-04:** plugin-git-sync Phase 4 (multi-language linking UI). Backend foundation **shipped** (Session 1): migration `e8f9a0b1c2d3` adds `books.translation_group_id` (nullable indexed UUID). `app/services/translation_groups.py` owns the linking primitives - `derive_language(branch, metadata)` resolves `main-XX` → `XX` and bare `main` → `metadata.yaml.language`; `link_books` creates a fresh group or folds members into the lexicographically-smallest existing group (deterministic merge); `unlink_book` clears the row and auto-unlinks the lone survivor of a two-book group; `list_siblings` excludes self + soft-deleted, sorted by language. Endpoints `GET /api/translations/{book_id}` (siblings + group id), `POST /api/translations/link` (group two or more books), `POST /api/translations/{book_id}/unlink` (idempotent 204). 23 backend tests cover every primitive + endpoint + 404/422 paths. **Remaining (Session 2):** multi-branch auto-import on git URL (enumerate `main` + `main-XX` branches and create a linked book per branch), frontend metadata editor "Translations:" row with sibling navigation, Settings link/unlink UI, 8-language i18n. Estimated remaining: 5-8h.
