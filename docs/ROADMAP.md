...
- [ ] **PGS-02:** plugin-git-sync Phase 2 (export to repo, overwrite MVP). Backend foundation **shipped** (Session 1): migration `d7e8f9a0b1c2` adds `git_sync_mappings` table, `app/services/git_sync_mapping.py` persists the clone to `uploads/git-sync/{book_id}/repo/` after a successful git import + writes the mapping, `app/services/git_sync_commit.py` re-scaffolds the book via plugin-export's `scaffold_project` and creates one commit on the local clone with overwrite semantics (variant 2c). Endpoints `GET/POST /api/git-sync/{book_id}` (status + commit). 10 backend tests cover persistence + happy path + 404/410/409/501 error paths. **Remaining (Session 2):** frontend "Commit to Repo" button + status polling + dirty-warning dialog + 8-language i18n; push wiring (reuses core PAT handling) currently returns 501.
