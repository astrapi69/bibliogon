...
- [ ] **CIO-07:** `.git/` adoption in import wizard (Option C from audit: opt-in with 3 radio choices). **Backend session 1 shipped.** Frontend wizard UI + i18n + help docs translations in session 2.
  ...
  - Help doc `docs/help/en/import/git-adoption.md`. DE translation deferred to session 2.
  - Shipped commits `adb031d..d11d582` (and the backfill consolidation). Backend 953 passed / 1 skipped (one order-dependent failure in the isolated detect-with-dot-git test noted for session-2 follow-up, passes in isolation).
