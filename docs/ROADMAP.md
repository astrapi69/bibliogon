...
- [ ] **PGS-03:** plugin-git-sync Phase 3 (smart-merge on re-import). Backend foundation **shipped** (Session 1): `app/services/git_sync_diff.py` runs the three-way comparison. Reads base + remote WBT chapters from arbitrary git refs via `git ls-tree` + `git show` (no working-tree checkout), reads the local DB and converts each chapter through `bibliogon_export.tiptap_to_md.tiptap_to_markdown` so what diff sees matches what commit-to-repo would write. Identity is `(section, slug-of-title)`; the pure `_classify` covers every classification (unchanged / remote_changed / local_changed / both_changed / remote_added / local_added / remote_removed / local_removed) plus the same-edit-on-both-sides not-a-conflict case and a normalize step that tolerates blank-line runs and trailing newlines. Endpoint `POST /api/git-sync/{book_id}/diff` returns the per-chapter classifications + a counts summary. 16 backend tests cover every classification + endpoint 404/410. **Remaining (Session 2):** frontend conflict resolution UI (Keep Bibliogon / Take from repo / Mark conflict), apply-resolution endpoint that mutates the DB or stages the chosen markdown into the clone, and integration into the re-import wizard flow. Estimated remaining: 8-12h.
- [ ] **PGS-04:** plugin-git-sync Phase 4 (multi-language linking UI). Branch-driven detection: branches matching the `main-XX` pattern derive `language=XX` directly from the branch name; the bare `main` branch reads `metadata.yaml.language` for the actual language. The author maintains `metadata.yaml` correctly (the `language` field is always set), so no fallback heuristics or user prompts are needed during import. **Cross-link model:** post-import the books are linked as translations of each other - **no master/translation hierarchy** - via a new `Book.translation_group_id` column auto-populated on import. The metadata editor surfaces a "Translations: [DE, FR, ES]" row with one-click navigation to each sibling book. Manual link/unlink in Settings remains available for books that were not imported as a multi-branch group. Estimated effort: 8-12h.
