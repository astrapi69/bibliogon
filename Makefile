.PHONY: dev dev-bg dev-bg-logs dev-down dev-backend dev-frontend stop restart fix-watchers \
       ...
       check-blockers archive-task archive-task-dry install-hooks verify-learnset-schema verify-learnset-upstream import-portfolio import-portfolio-check repair-image-paths agent-db \
...
# --- Code-level SAST (mirrors the bandit step in ci.yml) ---

security-backend: bandit-backend audit-backend ## bandit SAST + pip-audit (backend code + dependency security)
...
agent-db: ## Clone the dev database into an agent-local copy (#794). Add FORCE=1 to refresh.
	@python3 scripts/agent_db_snapshot.py $(if $(FORCE),--force,)
