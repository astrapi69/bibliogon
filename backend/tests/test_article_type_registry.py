...
from app.schemas import ArticleType
from app.services import article_type_registry
from app.services.article_type_registry import (
    ArticleTypeDef,
    article_type_extra_field_names,
    article_type_ids,
    default_article_type_id,
    get_article_type,
    load_article_types,
)
...
def fake_registry_path(
    ...
    path = tmp_path / "article-types.yaml"
    monkeypatch.setattr(article_type_registry, "_REGISTRY_PATH", path)
    return path
...
class TestRealRegistry:
    ...
    def test_each_type_is_a_validated_ArticleTypeDef(self) -> None:
        for type_id, definition in load_article_types().items():
            assert isinstance(definition, ArticleTypeDef)
            assert definition.id == type_id

    def test_blogpost_is_default(self) -> None:
        blogpost = get_article_type("blogpost")
        assert blogpost is not None
        assert blogpost.default is True
        assert blogpost.icon == "FileText"
        assert blogpost.extra_fields == []

    def test_default_article_type_id_returns_blogpost(self) -> None:
        assert default_article_type_id() == "blogpost"

    def test_tutorial_extra_fields_match_known_truth(self) -> None:
        tutorial = get_article_type("tutorial")
    ...
    def test_review_rating_bounds_are_1_to_5(self) -> None:
        review = get_article_type("review")
    ...
    def test_article_type_extra_field_names_for_tutorial(self) -> None:
        assert article_type_extra_field_names("tutorial") == frozenset(
            {
                "difficulty_level",
                "prerequisites",
                "estimated_duration_minutes",
    ...
    def test_extra_field_names_for_unknown_type_returns_empty(self) -> None:
        assert article_type_extra_field_names("unknown_type") == frozenset()

    def test_extra_field_names_for_blogpost_is_empty(self) -> None:
        assert article_type_extra_field_names("blogpost") == frozenset()

    def test_get_unknown_article_type_returns_none(self) -> None:
        assert get_article_type("cookbook") is None

    def test_literal_matches_registry(self) -> None:
        """SSoT verification: the Pydantic Literal in
        ``app.schemas.ArticleType`` MUST equal the registry's id set.

        Same drift-detector pattern as BookType. A new article_type
        in YAML without a Literal bump fails this test.
        """
        from typing import get_args

        literal_values = set(get_args(ArticleType))
        registry_ids = set(load_article_types().keys())
        assert literal_values == registry_ids, (
            f"ArticleType Literal {literal_values} does not match "
            f"registry ids {registry_ids}. Update "
            f"backend/app/schemas/__init__.py:ArticleType + "
            f"frontend/src/api/client.ts:ArticleType to match the YAML."
        )
...
class TestFakedRegistry:
    ...
    def test_missing_article_types_key_returns_empty(
        self, fake_registry_path: Path
    ) -> None:
        fake_registry_path.write_text(
            "unrelated_key: value\n", encoding="utf-8"
    ...
    def test_article_types_not_list_returns_empty(
        self, fake_registry_path: Path
    ) -> None:
        fake_registry_path.write_text(
            "article_types: not_a_list\n", encoding="utf-8"
        )
        assert load_article_types() == {}
    ...
    def test_default_falls_back_to_first_id_when_none_marked(
        ...
        assert default_article_type_id() == "aaa"
    ...
    def test_extra_field_extra_keys_forbidden(
        ...
        # ``extra="forbid"`` on ArticleTypeExtraField means a typo'd
        ...
        assert get_article_type("bad_field") is None
