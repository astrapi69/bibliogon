...
def test_missing_secret_falls_back_to_autogenerated(tmp_path, monkeypatch):
    # Regression for #35: with NEITHER BIBLIOGON_CREDENTIALS_SECRET nor
    # BIBLIOGON_SECRET_KEY set, the store auto-generates + persists a secret
    # so credential storage works out of the box (no RuntimeError -> no 500).
    # Save + load round-trips with the same auto-generated key.
    monkeypatch.delenv("BIBLIOGON_CREDENTIALS_SECRET", raising=False)
    monkeypatch.delenv("BIBLIOGON_SECRET_KEY", raising=False)
    save_encrypted(VALID_SA, credentials_dir=tmp_path)
    assert load_decrypted(credentials_dir=tmp_path) == VALID_SA


def test_credentials_secret_takes_precedence_over_secret_key(tmp_path, monkeypatch):
    # The dedicated var wins over the app secret (independent rotation).
    monkeypatch.setenv("BIBLIOGON_CREDENTIALS_SECRET", "dedicated")
    monkeypatch.setenv("BIBLIOGON_SECRET_KEY", "app-secret")
    save_encrypted(VALID_SA, credentials_dir=tmp_path)
    # Decrypt only succeeds with the dedicated secret still set.
    assert load_decrypted(credentials_dir=tmp_path) == VALID_SA
    # Switching to ONLY the app secret can no longer decrypt (different key).
    monkeypatch.delenv("BIBLIOGON_CREDENTIALS_SECRET", raising=False)
    with pytest.raises(RuntimeError, match="decrypt"):
        load_decrypted(credentials_dir=tmp_path)
