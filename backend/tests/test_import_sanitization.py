...
def test_import_single_markdown_sanitizes_content():
    ...
    with TestClient(app) as client:
        ...
        r = client.post(
            ...
            files={"file": ("chapter.md", io.BytesIO(dirty.encode("utf-8")), "text/markdown")},
        )
        assert r.status_code == 200, r.text
        book_id = r.json()["result"]["book_id"]
        try:
...
def test_import_markdown_zip_sanitizes_all_chapters():
    """Each chapter inside a markdown ZIP is sanitized."""
    with TestClient(app) as client:
        buf = io.BytesIO()
        with zipfile.ZipFile(buf, "w") as zf:
            zf.writestr("ch1.md", "# Eins\n\nA\u00a0B")
            zf.writestr("ch2.md", "# Zwei\n\nC\u200bD")
        buf.seek(0)
