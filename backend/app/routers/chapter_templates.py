...
from app.repositories.chapter_templates import (
    ChapterTemplateRepository,
    get_chapter_template_repository,
)
...
def list_chapter_templates(
    repo: ChapterTemplateRepository = Depends(get_chapter_template_repository),
):
...
def create_chapter_template(
    ...
    _validate_child_ids(repo, child_ids)
...
def update_chapter_template(
    ...
    if "child_template_ids" in fields:
        ...
        _validate_child_ids(repo, new_children, self_id=template.id)
...
def _validate_child_ids(
    repo: ChapterTemplateRepository,
    child_ids: list[str],
    *,
    ...
    found_ids = repo.existing_ids(child_ids)
    ...
    if self_id is not None:
        ...
        while stack:
            ...
            if current in seen:
                continue
            seen.add(current)
            grand = _decode_child_ids(repo.child_template_ids_raw(current))
            if self_id in grand:
                raise HTTPException(
                    status_code=400,
...
async def import_chapter_template(
    file: UploadFile,
    repo: ChapterTemplateRepository = Depends(get_chapter_template_repository),
):
    ...
    _validate_child_ids(repo, raw_child_ids)
    ...
    created = repo.add(template)
    logger.info("Imported chapter template %s (%s)", created.id, created.name)
    return created
