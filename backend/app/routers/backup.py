...
def _serialize_book_for_backup(book: Book) -> dict[str, Any]:
...
def _restore_book_from_data(book_data: dict[str, Any]) -> Book:
...
def export_backup(db: Session = Depends(get_db)):
    ...
    for book in books:
        ...
        book_data = _serialize_book_for_backup(book)
...
def import_backup(file: UploadFile, db: Session = Depends(get_db)):
    ...
    try:
        ...
        for book_dir in sorted(books_dir.iterdir()):
            ...
            book = _restore_book_from_data(book_data)
            db.add(book)
